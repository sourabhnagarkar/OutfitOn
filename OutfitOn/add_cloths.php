    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .left-panel {
            flex: 1 1 300px;
            min-height: 250px;
            background: url('cloth.jpeg') center/cover no-repeat;
...
<div class="container">
    ...
    <form name="registerForm" action="add_cloths_check.php" method="POST" enctype="multipart/form-data" onsubmit="return validatePassword();">

        <h2>Add Cloth</h2>
        <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">

        <label>Cloth Name:</label>
        <input type="text" name="name" id="name" required pattern="[A-Za-z\s]+" title="Only letters and spaces allowed" />

        <label>Description:</label>
        <textarea name="description" id="description" rows="3" required></textarea>

        <label>Category:</label>
        <select name="category" id="category" required onchange="loadCities()">
