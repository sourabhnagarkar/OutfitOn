...
// Fetch user details
$stmt = $conn->prepare("SELECT name, email, phone, address, city, state FROM users WHERE id = ?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    echo "<script>alert('User details not found.'); window.location.href='index.php';</script>";
    exit();
}
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .left-panel {
            flex: 1 1 300px;
            min-height: 250px;
            background: url('OrderPlace.jpeg') center/cover no-repeat;
...
<div class="topbar">
    ...
    <?php if (isset($_SESSION['user'])): ?>
        <div class="dropdown">
            <button class="dropdown-button">👤 <?php echo htmlspecialchars($_SESSION['user']['name']); ?></button>
...
<div class="container">
    <div class="left-panel">
     ...
     <form action="place_order.php" method="post">
        ...
        <label>City:</label>
        <input type="text" name="city" value="<?php echo htmlspecialchars($user['city']); ?>" readonly>

        <label>State:</label>
        <input type="text" name="state" value="<?php echo htmlspecialchars($user['state']); ?>" readonly>
<br>
		<label>From :</label>
